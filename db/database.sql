CREATE DATABASE IF NOT EXISTS giraffe;
USE giraffe;
CREATE TABLE sqli(id int NOT NULL AUTO_INCREMENT, username varchar(255), password char(32), profile text, PRIMARY KEY (id));
CREATE USER IF NOT EXISTS 'sqli'@'%' IDENTIFIED BY 'this_is_a_password';
UPDATE mysql.user SET Host='%' WHERE User='sqli';
UPDATE mysql.db SET Host='%' WHERE User='sqli';
FLUSH PRIVILEGES;
GRANT ALL ON giraffe.* TO 'sqli'@'%';
